...
class Session extends EventEmitter {
  ...
  connect() {
    ...
    this[connectionSymbol] =
        binding.connect((message) => this[onMessageSymbol](message));
  }
...
class InspectorState extends URL {
  constructor(rawString) {
    super(rawString);
    // adding a convenience properties that are specific for inspector state
    this.UUID = this.path;
    this.toURL = this.toString;
  }

  static currentState() {
    const u = binding.url();
    if (!u) return null;
    return new InspectorState(u);
  }
}

module.exports = {
  open: (port, host, wait) => binding.open(port, host, !!wait),
  close: process._debugEnd,
  currentState: InspectorState.currentState,
  Session
};
